/**
 * Point Kalkulator BMI nav/logo to /kalkulator-bmi; logo home → dieta.
 * node scripts/patch-kalkulator-bmi-url.mjs
 */
import fs from 'fs';
...
function patch(html, rel) {
    ...
    // Nav: Kalkulator BMI href="/" or index → kalkulator-bmi
    out = out.replace(
        /(<a class="nav-link[^"]*" href=")\/("(?:>|[^>]*>)Kalkulator BMI<\/a>)/g,
        '$1kalkulator-bmi$2'
    );
    out = out.replace(
        /(<a class="nav-link[^"]*" href=")index\.html("(?:>|[^>]*>)Kalkulator BMI<\/a>)/g,
        '$1kalkulator-bmi$2'
    );
    out = out.replace(
        /(<a class="nav-link[^"]*" href=")(\.\.\/)("(?:>|[^>]*>)Kalkulator BMI<\/a>)/g,
    ...
    // Body links
    out = out.replace(/(<a href="\/">)Kalkulator(?: BMI)?(<\/a>)/g, '$1Kalkulator BMI$2');
    out = out.replace(/<a href="\/">Kalkulator BMI<\/a>/g, '<a href="kalkulator-bmi">Kalkulator BMI</a>');
...
console.log(`Updated Kalkulator BMI URLs / logos in ${n} files.`);

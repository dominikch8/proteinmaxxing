...
export function buildHealthDisclaimer(prefix = '', opts = {}) {
    ...
    if (compact) {
        ...
        <p><strong>Informacja:</strong> Proteiner to narzędzie edukacyjne, nie porada medyczna. Wyniki kalkulatora i dane w bazie mają charakter orientacyjny. Przed zmianą diety skonsultuj się z lekarzem lub dyplomowanym dietetykiem. <a href="${about}">O autorze</a> · <a href="${informacje}">Polityka</a></p>
    </aside>`;
    }
    ...
    return `    <aside class="health-disclaimer" role="note" aria-labelledby="health-disclaimer-heading">
        ...
        <p><a href="${about}">Kim jest autor strony</a> · <a href="${poradnik}">Poradnik żywienia</a> · <a href="${informacje}">Polityka prywatności</a></p>
    </aside>`;
}
